...
이 저장소에는 `master` 브랜치 push 시 Cloud Run `auth` / `remote` 서비스와 국내 주문이력 배치 Job + Scheduler를 순서대로 배포하는 GitHub Actions workflow가 포함되어 있습니다.

- workflow 파일: [.github/workflows/deploy-cloud-run.yml](./.github/workflows/deploy-cloud-run.yml)
- 기본 흐름: `push to master -> uv run pytest -> auth deploy -> remote deploy -> batch deploy -> scheduler deploy`
