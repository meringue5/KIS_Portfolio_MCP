...
V2_DATA_OBJECTS = (
    ...
    DataObject("trade_thread_command_revisions", "table", "silver", "silver", "append-only owner command revisions", "parquet", "confidential", "Audited owner thread metadata lot-link and sell-allocation intents.", "one accepted owner thread command revision", "thread_id, revision; actor, client, idempotency key hash is unique", "silver"),
    DataObject("trade_thread_risk_plan_revisions", "table", "silver", "silver", "append-only owner revisions", "parquet", "confidential", "Owner-authoritative typed thread stop reference and risk budget history.", "one thread risk-plan revision", "risk_plan_id, revision", "silver"),
    DataObject("trade_thread_risk_plans_current", "view", "silver", "silver", "latest knowledge projection", "excluded", "confidential", "Latest known typed risk plan for each investment thread.", "one current thread risk plan", "thread_id", "silver"),
    ...
    DataObject("filing_events", "table", "silver", "silver", "legacy empty foundation", "parquet", "internal", "Preserved pre-ADR-025 filing foundation; migration 0014 requires zero rows.", "legacy issuer filing document version", "issuer_id, filing_id, document_version", "silver"),
    DataObject("financial_facts", "table", "silver", "silver", "legacy empty foundation", "parquet", "internal", "Preserved pre-ADR-025 fact foundation; migration 0014 requires zero rows.", "legacy filing taxonomy fact", "issuer_id, filing_id, taxonomy, concept, period_end, unit, dimension_hash", "silver"),
    DataObject("issuer_alias_revisions", "table", "silver", "silver", "append-only revisions", "parquet", "internal", "Bitemporal official issuer alias evidence and relation quality.", "one source alias content revision and validity interval", "source_id, alias_type, alias_value, source_valid_from, revision_hash", "silver"),
    DataObject("issuer_source_aliases_current", "view", "silver", "silver", "latest knowledge projection", "excluded", "internal", "Latest governed revision for each filing-source alias.", "one source alias", "source_id, alias_type, alias_value", "silver"),
    DataObject("filing_identities", "table", "silver", "silver", "append-only identities", "parquet", "internal", "Stable source filing identities bound to verified issuers.", "one source filing identity", "source_id, jurisdiction, source_filing_id", "silver"),
    DataObject("filing_revisions", "table", "silver", "silver", "append-only content revisions", "parquet", "internal", "Immutable official filing content, correction and dual-clock revisions.", "one stable filing identity and content hash", "filing_identity_id, content_hash", "silver"),
    DataObject("filing_revisions_current", "view", "silver", "silver", "latest revision projection", "excluded", "internal", "Latest filing content with verified supersession status.", "one source filing identity", "filing_identity_id", "silver"),
    DataObject("financial_fact_revisions", "table", "silver", "silver", "append-only fact revisions", "parquet", "internal", "Immutable source taxonomy facts retaining lexical values and dimensional context.", "one filing content source fact revision", "filing_revision_id, taxonomy, concept, period_start, period_end, unit, statement_scope, dimension_hash, fact_revision_hash", "silver"),
    DataObject("financial_fact_revisions_current", "view", "silver", "silver", "latest fact projection", "excluded", "internal", "Latest source fact revision without overwriting source taxonomy.", "one filing content taxonomy fact context", "filing_revision_id, taxonomy, concept, period_start, period_end, unit, statement_scope, dimension_hash", "silver"),
    DataObject("dividend_events", "table", "silver", "silver", "legacy empty foundation", "parquet", "confidential", "Preserved pre-ADR-026 dividend foundation; migration 0015 requires zero rows.", "legacy mutable dividend state event", "dividend_event_id", "silver"),
    DataObject("dividend_actions", "table", "silver", "silver", "append-only identities", "parquet", "internal", "Stable issuer and instrument dividend action identities.", "one source dividend action identity", "source_id, jurisdiction, issuer_id, instrument_id, source_action_id", "silver"),
    DataObject("dividend_action_revisions", "table", "silver", "silver", "append-only content revisions", "parquet", "internal", "Immutable dividend terms, dates, status and correction revisions.", "one action content revision", "dividend_action_id, revision_hash", "silver"),
    DataObject("dividend_actions_current", "view", "silver", "silver", "latest knowledge projection", "excluded", "internal", "Latest known revision for each dividend action.", "one dividend action", "dividend_action_id", "silver"),
    DataObject("dividend_entitlements", "table", "silver", "silver", "append-only identities", "parquet", "confidential", "Stable account entitlement identities for dividend actions.", "one dividend action and account", "dividend_action_id, account_id", "silver"),
    DataObject("dividend_entitlement_revisions", "table", "silver", "silver", "append-only revisions", "parquet", "confidential", "Source-confirmed or explicitly estimated entitlement and coverage revisions.", "one entitlement evidence revision", "dividend_entitlement_id, revision_hash", "silver"),
    DataObject("dividend_entitlements_current", "view", "silver", "silver", "latest knowledge projection", "excluded", "confidential", "Latest known entitlement coverage for each action and account.", "one dividend entitlement", "dividend_entitlement_id", "silver"),
    DataObject("dividend_receipt_links", "table", "silver", "silver", "append-only identities", "parquet", "confidential", "Stable relations between dividend actions, entitlements and cash facts.", "one action relation identity", "dividend_action_id, relation_key", "silver"),
    DataObject("dividend_receipt_link_revisions", "table", "silver", "silver", "append-only reversible revisions", "parquet", "confidential", "Versioned exact, reconciled, candidate, partial, unmatched, gap and reversal states.", "one receipt-link knowledge revision", "dividend_receipt_link_id, revision_hash", "silver"),
    DataObject("dividend_receipt_links_current", "view", "silver", "silver", "latest knowledge projection", "excluded", "confidential", "Latest receipt-link state without mutating monetary cash facts.", "one receipt-link identity", "dividend_receipt_link_id", "silver"),
    DataObject("cash_flow_event_amount_components", "table", "silver", "silver", "append-only sourced revisions", "parquet", "confidential", "Source-provided gross, tax and net components of immutable cash events.", "one cash event component revision", "cash_flow_event_id, component_type, revision", "silver"),
    DataObject("macro_observations", "table", "silver", "silver", "legacy empty foundation", "parquet", "internal", "Preserved pre-ADR-027 macro foundation; migration 0016 requires zero rows.", "legacy series period vintage revision", "series_contract_id, observation_period, realtime_start, source_revision", "silver"),
    DataObject("macro_observation_revisions", "table", "silver", "silver", "append-only typed revisions", "parquet", "internal", "Heterogeneous provider-vintage and observed-content macro revisions.", "one governed series period and typed revision", "series_contract_id, observation_period, revision_kind, revision_key", "silver"),
    DataObject("macro_observations_current", "view", "silver", "silver", "latest knowledge projection", "excluded", "internal", "Latest system-known macro revision per series and period.", "one governed series and observation period", "series_contract_id, observation_period", "silver"),
    DataObject("macro_observations_as_of", "view", "silver", "silver", "knowledge interval projection", "excluded", "internal", "Macro revisions with the next system knowledge boundary for PIT selection.", "one governed series period and knowledge interval", "series_contract_id, observation_period, revision_kind, revision_key", "silver"),
    DataObject("alpha_vantage_consensus_forward_snapshots", "table", "silver", "silver", "append-only normalized snapshots", "parquet", "restricted", "Owner-only forward consensus rows with fetched-at knowledge and no raw provider payload.", "one issuer forecast period metric horizon and fetched-at", "issuer_id, provider_forecast_date, metric, horizon, fetched_at", "silver"),
    DataObject("alpha_vantage_consensus_forward_latest", "view", "silver", "silver", "latest fetched-at projection", "excluded", "restricted", "Latest owner-only Alpha forward consensus row without historical-provider PIT claims.", "one issuer forecast period metric and horizon", "issuer_id, provider_forecast_date, metric, horizon", "silver"),
    ...
    DataObject("alert_candidates", "table", "gold", "gold", "idempotent append; conflicting replay rejected", "parquet", "confidential", "Quality-gated point-in-time alert evaluations with redacted public context.", "one rule version subject session and evaluation slot", "candidate_id; rule_id, rule_version, subject_type, subject_id, session_key, evaluation_slot is unique", "gold"),
    ...
    DataObject("dividend_monthly_native", "view", "gold", "gold", "received-cash rebuild view", "excluded", "confidential", "Native-currency monthly dividend receipts and coverage.", "one month, account, instrument and currency", "received_month, account_id, instrument_id, currency", "gold"),
    DataObject("dividend_monthly_krw", "view", "gold", "gold", "labeled FX rebuild view", "excluded", "confidential", "Separately labeled governed-FX projection of monthly dividend receipts.", "one month, account, instrument and native currency", "received_month, account_id, instrument_id, currency", "gold"),
    DataObject("macro_profile_snapshots", "table", "gold", "gold", "rebuildable materialization", "parquet", "internal", "Point-in-time macro profile metrics, coverage, rights and lineage.", "one profile version evaluation cutoff and metric set", "profile_id, profile_version, evaluation_at, metric_set_version", "gold"),
    ...
    DataObject("market_calendar", "table", "control", "control", "upsert after explicit transition", "parquet", "internal", "V2-owned market open and close calendar; copied once from retained V1 reference data before V2-only maintenance.", "one market and trade date", "market, trade_date", "control"),
    DataObject("instrument_master", "table", "control", "control", "upsert after explicit transition", "parquet", "internal", "V2-owned KIS instrument reference used for domestic classification.", "one symbol and market", "symbol, market", "control"),
    DataObject("instrument_classification_overrides", "table", "control", "control", "owner-approved upsert after explicit transition", "parquet", "confidential", "V2-owned exposure classification overrides.", "one symbol and market", "symbol, market", "control"),
    ...
    DataObject("alert_rule_versions", "table", "control", "control", "immutable version registration", "parquet", "confidential", "Approved alert rules, validity, severity floor and disabled or shadow delivery mode.", "one alert rule version", "rule_id, version", "control"),
    DataObject("alert_state_revisions", "table", "control", "control", "append-only state transitions", "parquet", "confidential", "Deterministic alert episode, escalation, recovery and re-entry history.", "one alert identity transition revision", "alert_identity, revision; candidate_id is unique", "control"),
    DataObject("alert_candidate_outcomes", "table", "control", "control", "append-only evaluation outcomes", "parquet", "confidential", "Exactly-once candidate processing result for transition, no-change or quality suppression.", "one alert candidate", "candidate_id", "control"),
    DataObject("alert_states_current", "view", "control", "control", "latest revision projection", "excluded", "confidential", "Latest governed state for each alert identity.", "one alert identity", "alert_identity", "control"),
    DataObject("alert_dispatch_claims", "table", "control", "control", "idempotent leased claim", "parquet", "confidential", "Transport-neutral delivery claim with opaque destination and hashed lease token.", "one candidate channel and destination", "candidate_id, channel, destination_ref", "control"),
    DataObject("alert_delivery_attempts", "table", "control", "control", "append-only attempts", "parquet", "confidential", "Redacted delivery result including terminal unknown state without response bodies.", "one dispatch attempt", "dispatch_id, attempt_no", "control"),
    DataObject("alert_calibration_runs", "table", "control", "control", "immutable report plus owner review gate", "parquet", "confidential", "Bounded replay report, asset-class threshold proposal and explicit historical provenance limits.", "one rule-set replay report hash", "calibration_run_id; report_hash is unique", "control"),
    DataObject("alert_shadow_windows", "table", "control", "control", "bounded evidence upsert", "parquet", "confidential", "Two-week DB-only slot coverage, de-duplication, quality, sensitive audit and zero-send evidence.", "one rule-set shadow window", "shadow_window_id", "control"),
    DataObject("alert_rule_approval_revisions", "table", "control", "control", "append-only owner decisions", "parquet", "confidential", "Owner approval, rejection or revocation tied to calibration and verified shadow evidence.", "one rule version decision revision", "rule_id, rule_version, revision", "control"),
    ...
    DataObject("owner_review_items", "table", "control", "control", "append-only identities", "parquet", "confidential", "Stable owner-review identities for missing plans journals and allocation intent.", "one review subject and review type", "review_item_id; identity_hash is unique", "control"),
    DataObject("owner_review_item_revisions", "table", "control", "control", "append-only state revisions", "parquet", "confidential", "Versioned open answered or dismissed owner-review states.", "one review item state revision", "review_item_id, revision", "control"),
    DataObject("owner_review_items_current", "view", "control", "control", "latest knowledge projection", "excluded", "confidential", "Latest state for each owner review item.", "one current review item", "review_item_id", "control"),
    ...
    DataObject("fundamental_concept_mappings", "table", "control", "control", "append-only reviewed versions", "parquet", "internal", "Versioned source-taxonomy to normalized-concept mappings.", "one mapping identity and version", "mapping_id, version", "control"),
    DataObject("fundamental_concept_mappings_current", "view", "control", "control", "latest mapping projection", "excluded", "internal", "Latest known version of each concept mapping.", "one mapping identity", "mapping_id", "control"),
    DataObject("macro_series_definitions", "table", "control", "control", "immutable definition projection", "parquet", "internal", "Exact approved macro series definitions and contract hashes.", "one series contract version", "series_contract_id, version", "control"),
