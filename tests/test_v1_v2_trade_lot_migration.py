...
def test_trade_lot_thread_migration_is_idempotent(tmp_path: Path) -> None:
    ...
    c.execute(f"""COPY (SELECT * FROM (VALUES
      ('ria',date '2026-01-02','1','100','090000',timestamp '2026-01-03',10::BIGINT,1000::BIGINT,1000::BIGINT,'005930',''),
      ('ria',date '2026-01-02','1','101','090100',timestamp '2026-01-03',0::BIGINT,1000::BIGINT,1000::BIGINT,'005930','')
    ) t(account_type,order_date,order_branch_no,order_no,order_time,last_seen_at,filled_qty,avg_price,order_price,symbol,original_order_no)) TO '{backup/'domestic_orders.parquet'}' (FORMAT PARQUET)""");c.close()
    ...
    assert first==second
    assert first["source_orders"]=={"rows":2,"filled":1,"deferred_unfilled":1,"corrections":0}
    assert first["target_rows"]=={"trade_events":1,"purchase_lots":1,"threads":1,"thread_links":1}
