...
| 8 | WI-043 Remote MCP managed commands | V2-W0604/0605 | WI-024, WI-042 | in progress; isolated typed command ports, fixed-job/idempotency and append-only journal/thread revision fixtures only; production effects none |
...
- implementation gate: MS-002가 최소 `stabilizing`이면 MS-003은 `in_progress`로 전환한다. WI-046 cutover를
  ...
  `verified`이며 WI-039, WI-041과 WI-042도 isolated verification을 마쳤다. WI-043이 현재 dependency-ready
  isolated 구현 단위로 활성화됐다.
