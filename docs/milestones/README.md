...
| 현재 단일 WIP | MS-004 `WI-032` in progress; final repository-only V2 documentation truth cutover |
...
표의 dependency 설명은 완료 당시의 실행 순서를 보존한다. 현재 MS-002와 MS-003은 닫혔고, MS-004의
마지막 repository-only 문서 gate인 WI-032만 진행 중이다.
