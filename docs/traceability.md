...
| --- | --- | --- | --- | --- | --- |
| 실제 Claude가 `000660`, `dataset.price-bar-daily`, `portfolio-refresh`를 조회했을 때 데이터·증거·식별자 실패가 모두 `no_governed_rows`로 보이는 문제를 교정 | existing DEC-029/031/032/034/038 and ADR-020/021/028; no scope expansion | WI-058 discovered from WI-042/WI-046 | public identifier resolver, precise unavailable reasons, direct exposure query, per-run price quality evidence, real-input regression | PR #111 / `8e09c45`; Remote run 34915731283 plus controlled revision 00046 traffic promotion; V2 core Job run 34916315077; canonical/tag health 200 and unauthenticated MCP 401 | stabilizing; owner authenticated Claude read acceptance pending |
