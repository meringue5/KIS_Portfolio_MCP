...
rate limit 응답은 같은 runtime scope 전체에 adaptive cooldown을 적용한 뒤 한 번만 재시도한다. 첫 거부는
1초, 반복 거부는 2초, 4초 순서로 감속하며 기본 최대값은 10초다. 다른 coroutine도 같은 cooldown을
공유하므로 각 요청이 독립적으로 재시도하며 호출 폭주를 만드는 일을 막는다. 재시도 후에도 거부되면
`KISRateLimitError`로 종료한다. timeout, bulkhead, circuit breaker, stale fallback 계약은
[KIS API Resilience](./kis-api-resilience.md)를 따른다.
...
| `KIS_RATE_LIMIT_MAX_COOLDOWN_SECONDS` | `10.0` | 반복 유량 거부 시 process-wide 최대 cooldown |
