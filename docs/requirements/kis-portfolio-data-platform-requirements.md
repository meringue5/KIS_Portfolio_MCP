...
- caption은 총자산 요약, 계좌 구성, 자산 구성과 Top 5를 명시적인 구획으로 나누고, 금액·비중 열은
  Telegram HTML이 지원하는 고정폭 서식으로 정렬한다. 이미지와 같은 값을 반복하더라도 모바일에서
  빠르게 훑을 수 있는 compact table 형태를 우선하며 장식보다 숫자 열의 일관성을 유지한다.
