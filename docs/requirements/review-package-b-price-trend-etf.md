...
### B-1. 가격 원장 — 수정주가를 분석 기준으로 하고 원주가도 잃지 않는다
...
### B-2. 추세·RSI·거래량 — 서로 다른 기간 개념을 섞지 않는다
...
### B-3. 고점 — 보유기간 고점을 주 지표로 하고 ATH라는 이름을 엄격히 쓴다
...
### B-4. ETF look-through — KRX/운용사 PDF를 canonical로, KIS를 보조로 사용한다
...
| 수정·원주가 dual basis | 분석과 감사 모두 가능 | logical key와 migration 필요 | **권고** |
| RSI 20·50·120 | 이동평균과 숫자가 같아 보임 | RSI 의미와 민감도 혼동 | 비권고 |
| RSI14 + SMA20·50·120 | 역할이 분명하고 재현 가능 | 지표 종류가 둘로 나뉨 | **권고** |
| 3년 최고를 ATH로 표시 | 구현이 쉬움 | 상장 이후 ATH로 오인 | 비권고 |
| 보유 에피소드 고점 중심 | 사용자 손익 맥락과 일치 | episode identity 필요 | **권고** |
| KIS ETF 구성만 사용 | 인증·호출 경로 재사용 | 30행 제한으로 불완전 | 비권고 |
| KRX/운용사 PDF + KIS 검증 | 전체 구성과 빠른 cross-check | 파일 adapter·라이선스 검토 필요 | **권고** |

## 5. 승인 요청

아래 다섯 항목을 한 묶음으로 승인 요청한다.

| ID | 승인할 결정 | 권고 |
| --- | --- | --- |
| B-1 | 수정주가를 분석 기준으로 하고 원주가와 raw provenance를 병렬 보존 | 승인 |
| B-2 | SMA20·50·120, 거래량 20일 비율, Wilder RSI14를 v1 기본 지표 계약으로 사용 | 승인 |
| B-3 | 보유 에피소드 고점을 주 지표로 하고 3년 최고를 ATH라 부르지 않음 | 승인 |
| B-4 | KRX/운용사 PDF를 ETF canonical source로, KIS 상위 구성종목을 보조 검증으로 사용 | 승인 |
| B-5 | ETF 구성종목 일별 snapshot 3년 보존을 용량 기준선으로 채택 | 승인 |

승인되더라도 논리 계약만 확정한다. 실제 API 변경, schema migration, 3년 backfill, PDF 자동수집과
배포는 후속 구현 계획을 다시 승인받은 뒤 시작한다.
