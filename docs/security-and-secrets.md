...
| `KIS_TELEGRAM_TOTAL_ASSET_REPORT_ENABLED` | Config | V2 monitoring batch | No | env only | Defaults false. When enabled, the 10:00/16:00 digest may expose bounded percentages and safe instrument labels, never absolute assets, changes, account values or identifiers. |
