...
| OAuth dynamic client secret | auth server generated value | ChatGPT/Codex dynamic client token endpoint | Yes | hash in `oauth_clients` | Raw value is returned once to client and not recoverable from DB. Codex redirect admission is limited to a structurally validated IPv4 loopback callback. |
...
| `KIS_TELEGRAM_BOT_TOKEN` | Telegram BotFather, `kis-portfolio-telegram-bot-token` | V2 monitoring batch after WI-030-S02 activation | No | pinned Secret Manager version only | Rotate through BotFather, add a Secret Manager version and redeploy only the V2 monitoring jobs. |
| `KIS_TELEGRAM_CHAT_ID` | Owner-verified private bot conversation, `kis-portfolio-telegram-chat-id` | V2 monitoring batch after WI-030-S02 activation | No | pinned Secret Manager version only | Treat as a private destination secret; never store it in MotherDuck, logs, payload hashes or evidence. |
...
| `KIS_TELEGRAM_TOTAL_ASSET_REPORT_ENABLED` | Config | V2 monitoring batch | No | env only | Legacy DEC-054 percentage-only report flag. Defaults false and must never be enabled with the DEC-055 v2 flag. |
| `KIS_TELEGRAM_TOTAL_ASSET_REPORT_V2_ENABLED` | Config | V2 monitoring batch | No | env only | DEC-055 exact-value owner report flag. Defaults false; requires the approval flag and `dest.owner.primary`. |
| `KIS_TELEGRAM_OWNER_DESTINATION_APPROVED` | Config | V2 monitoring batch | No | env only | Explicit gate confirming that exact financial values may be sent to the already verified private owner destination. It is not a substitute for the pinned chat-ID secret. |
| `ALPHA_VANTAGE_API_KEY` | Alpha Vantage free personal account, `kis-portfolio-alpha-vantage-api-key` | WI-041 research only; no runtime consumer approved | No | Secret Manager version only | Research label only. Do not attach to Cloud Run/Jobs or persist provider payloads until source rights, PIT semantics, call budget and provider activation are separately approved. |
| `KOREA_EXIM_API_KEY` | Korea Eximbank public-data application, `kis-portfolio-korea-exim-api-key` | WI-060 owned-portfolio Jobs only | No | pinned Secret Manager version only | Never log or persist the key. Rotate by adding a version, changing `KOREA_EXIM_API_KEY_VERSION`, passing the source preflight and redeploying the three fixed-slot Jobs. |
