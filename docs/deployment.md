...
- 일반 `--source` GitHub Actions 배포는 `KIS_DEPLOY_RELEASE_ID=<sha12>-<run-id>-<attempt>`를 runtime
  ...
  service 배포가 성공하면 `gcloud run services update-traffic --to-latest`를 별도로 실행해 과거 수동
  traffic pin이 있어도 새 ready revision으로 100%를 전환한다. (`gcloud run deploy` 자체에는
  `--to-latest` 옵션이 없다.) 배포 뒤에는 service label뿐 아니라 `latestReadyRevisionName`, 실제 100% traffic revision과
