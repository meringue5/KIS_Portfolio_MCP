...
| `silver.trade_thread_command_revisions` | Remote MCP의 owner-authored thread metadata, explicit lot-link와 sell-allocation intent를 bearer/idempotency 원문 없이 보존하는 append-only audit revision | Parquet / confidential |
...
| `silver.dividend_events` | ADR-026 이전 빈 foundation; migration 0015가 0행을 preflight하고 자동 변환·삭제하지 않음 | Parquet / confidential |
| `silver.dividend_actions`, `silver.dividend_action_revisions`, `silver.dividend_actions_current` | issuer/instrument/source action identity, immutable terms/correction revision과 latest knowledge projection; account와 cash 금액 제외 | Parquet tables + rebuild view / internal |
...
| `control.pipeline_runs`, `control.pipeline_stage_runs` | logical run and resumable stage evidence; WI-055 digest는 date/slot/version별 build/send terminal outcome만 redacted 저장하고 send 시작 뒤 ambiguity를 재전송하지 않음; `dataset.pipeline-run-evidence`, `dataset.pipeline-stage-evidence` | Parquet / internal |
